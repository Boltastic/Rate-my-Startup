...
// Securely access the Gemini API Key
...
// 2. Main Startup Evaluation AI endpoint
app.post("/api/analyze", async (req, res) => {
  try {
    if (!apiKey) {
      res.status(500).json({
        error: "GEMINI_API_KEY is not defined. Please add your GEMINI_API_KEY in your Vercel project's Environment Variables dashboard under Settings -> Environment Variables."
      });
      return;
    }
...
Your assessment must strictly construct and return a detailed, realistic JSON assessment matching the required schema.
...
app.get("/api/savage-chat-stream", async (req, res) => {
  ...
  if (!apiKey) {
    res.status(500).send("data: " + JSON.stringify({ error: "GEMINI_API_KEY environment variable is missing on Vercel. Please add it to your project's Environment Variables." }) + "\n\n");
    return;
  }
